...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\linewidth]{Chương_3/results/stock_sentiment/01_a_label_dist.png}
        \caption{Biểu đồ số lượng nhãn: Sự chênh lệch đáng kể giữa Positive (1) và Negative (0)}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\linewidth]{Chương_3/results/stock_sentiment/01_b_textlen_dist.png}
        \caption{Phân phối mật độ số từ. Đa số câu đều ngắn hơn ngưỡng cắt \texttt{MAX\_LEN=50}}
    \end{minipage}
\end{figure}

\textbf{Phân tích Phân phối:} 
- Biểu đồ nhãn cho thấy sự mất cân bằng ($3685$ tích cực so với $2106$ tiêu cực). Điều này buộc chúng ta phải áp dụng kỹ thuật \texttt{class\_weights} trong quá trình học.
- Độ dài câu chủ yếu rơi vào khoảng 10-30 từ. Việc chọn tham số \texttt{MAX\_LEN = 50} là hoàn toàn an toàn (bao phủ hơn 99\% các câu) để thực hiện Zero-Padding.
...
\textbf{Đánh giá Ngữ nghĩa (Semantic Analysis):}
- \textbf{Negative (Màu đỏ):} Các từ nổi bật nhất là \textit{short} (bán khống), \textit{lower} (giảm), \textit{down}, \textit{loss}, \textit{fall}. Đây là những tín hiệu mạnh mẽ của thị trường gấu (Bearish).
- \textbf{Positive (Màu xanh):} Tràn ngập các từ biểu thị sự tăng trưởng như \textit{long} (mua dài hạn), \textit{buy}, \textit{high}, \textit{growth}, \textit{up}.
WordCloud chứng tỏ quá trình làm sạch văn bản (Data Cleaning) đã giữ lại được linh hồn của dữ liệu tài chính.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Chương_3/results/stock_sentiment/04_confusion_matrix.png}
...
Mô hình đạt độ chính xác (Accuracy) tổng thể là \textbf{76\%}. Tuy nhiên, Recall của lớp Tiêu cực (Negative) khá thấp (56\%), nghĩa là mạng RNN cơ bản khá khó khăn và dễ bị bỏ sót các tín hiệu dự báo thị trường giảm điểm. Lỗi phân loại này có thể do giới hạn về "bộ nhớ ngắn hạn" của SimpleRNN.
...
\textbf{Giải thích bản đồ Không gian nhúng t-SNE:} Các từ có cùng tính chất tài chính sẽ có xu hướng gom cụm (cluster) lại với nhau. Ví dụ, nhóm các mã ticker công ty hoặc nhóm các động từ xu hướng (trend verbs) sẽ kết nối khoảng cách rất gần với nhau, minh chứng mạng nơ-ron không chỉ học các ký tự vô hồn, mà đã thực sự hình thành được \textit{Không gian ngữ nghĩa (Semantic Space)} của thị trường chứng khoán trong quá trình tối ưu hóa. Tuy nhiên để đạt được F1-Score xuất sắc hơn cho mảng tài chính, cấu trúc SimpleRNN vẫn còn yếu và buộc chúng ta phải cân nhắc tới các kiến trúc phức tạp hơn như LSTM hay GRU.
